...
//
// The scaffolding (viewBox, space-gradient background, console frame, mono
// caption) is identical across scenes, so it lives in scene() below and each
// entry supplies only its caption + the body shapes. Two rules the builder keeps:
//   • Every gradient id is prefixed with the scene id (scene() builds `${id}-bg`).
//     Alpine keeps all stage panels in the DOM (x-show, not x-if), so the SVGs
//     coexist and an unprefixed gradient id would bleed across scenes.
...
// jobs, semantic red/amber for hazards, rust for Mars. viewBox is 400x150.

// Default space background and console-frame accent (overridable per scene).
...
//   body     required scene-specific shapes (drawn between frame and caption)
...
function scene(id, { caption, body, theme = 'teal', bg, frame, capColor }) {
  ...
  const [top, bottom] = bg || (honors ? COPPER_BG : SPACE);
  const frameColor = frame || (honors ? '#c0772f' : '#2a7d8a');
  const frameOpacity = frame ? 0.4 : honors ? 0.5 : 0.35;
  const cap = capColor || (honors ? '#e0b483' : '#9fc2c9');
  return `<svg viewBox="0 0 400 150" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">`
    ...
    + `<rect x="1.5" y="1.5" width="397" height="147" rx="9" fill="none" stroke="${frameColor}" stroke-width="1" opacity="${frameOpacity}"/>`
    + body
    + `<text x="16" y="138" font-family="monospace" font-size="10.5" letter-spacing=".05em" fill="${cap}">${caption}</text>`
    + `</svg>`;
}
...
export const SCENE_ART = {
  ...
  // ---------- C.8(A) mass <-> mole ----------
  'a-oxygen': scene('a-oxygen', { caption: 'CABIN O₂ · REPLACE WHAT THEY BREATHED', body: `
    ...
    <text x="55" y="88" text-anchor="middle" font-family="monospace" font-size="16" font-weight="700" fill="#e8f2f4">O₂</text>
  ...
  'a-fuel': scene('a-fuel', { caption: 'CH₄ BURN · LOAD THE RIGHT MASS', body: `
  ...
  'a-scrubber': scene('a-scrubber', { caption: 'TRAPPED CO₂ · CONVERT GRAMS TO MOLES', body: `
    ...
    <text x="69" y="40" text-anchor="middle" font-family="monospace" font-size="10" fill="#9fc2c9">CARTRIDGE</text>
    ...
    <text x="209" y="60" text-anchor="middle" font-family="monospace" font-size="10" fill="#e8f2f4">RECYCLER</text>
  ...
  'b-eva': scene('b-eva', { caption: 'EVA FILL · MOLES TO MOLECULES', body: `
    ...
    <rect x="262" y="40" width="116" height="60" rx="8" fill="#08141a" stroke="#3a5560" stroke-width="2"/>
    <text x="320" y="58" text-anchor="middle" font-family="monospace" font-size="9" fill="#79b0ba">SUIT O₂</text>
    <text x="320" y="80" text-anchor="middle" font-family="monospace" font-size="17" font-weight="700" fill="#2f8f5b">N × 10²³</text>
    <text x="320" y="93" text-anchor="middle" font-family="monospace" font-size="8.5" fill="#9fc2c9">molecules</text>` }),

  'b-ration': scene('b-ration', { caption: "DAY'S RATION · MOLES TO MOLECULES", body: `
    <rect x="34" y="30" width="120" height="44" rx="8" fill="#2a7d8a" stroke="#cfe6ea" stroke-width="1.5"/>
    <text x="94" y="57" text-anchor="middle" font-family="monospace" font-size="11" fill="#e8f2f4">PRINTER</text>
    ...
    <text x="232" y="92" text-anchor="middle" font-family="monospace" font-size="9.5" fill="#9fc2c9">C₆H₁₂O₆</text>
    <rect x="288" y="44" width="92" height="52" rx="6" fill="#08141a" stroke="#3a5560" stroke-width="2"/>
    <text x="334" y="62" text-anchor="middle" font-family="monospace" font-size="9" fill="#79b0ba">RATION LOG</text>
    <text x="334" y="82" text-anchor="middle" font-family="monospace" font-size="15" font-weight="700" fill="#2f8f5b">N × 10²³</text>` }),

  'b-sample': scene('b-sample', { caption: 'SCOOP · GRAMS TO FORMULA UNITS', bg: ['#0b1a22', '#241410'], body: `
    ...
    <text x="206" y="124" text-anchor="middle" font-family="monospace" font-size="9.5" fill="#9fc2c9">NaCl</text>
    <circle cx="338" cy="44" r="16" fill="#2a6b8a"/><path d="M328 40 q8 -6 18 2 q-6 8 -16 2 z" fill="#2f8f5b" opacity=".8"/>
  ...
  'c-ore': scene('c-ore', { caption: 'RUST ROCK · RICH ENOUGH TO SMELT?', body: `
    ...
    <text x="166" y="92" text-anchor="middle" font-family="monospace" font-size="12" font-weight="700" fill="#f1ddc8">Fe ?</text>
    ...
    <text x="292" y="33" text-anchor="middle" font-family="monospace" font-size="8" fill="#9fc2c9">% Fe</text>` }),

  'c-greenhouse': scene('c-greenhouse', { caption: 'FERTILIZER · REAL % NITROGEN?', body: `
    ...
    <path d="M62 110 q-2 -18 -12 -24 M62 110 q2 -16 12 -22 M62 110 V84" stroke="#2f8f5b" stroke-width="2.5" fill="none"/>
    <path d="M50 86 q-10 -2 -8 -10 q10 0 8 10z" fill="#2f8f5b"/><path d="M74 88 q10 -2 8 -10 q-10 0 -8 10z" fill="#2f8f5b"/>
    <path d="M88 110 q-2 -14 -10 -18 M88 110 V90" stroke="#2f8f5b" stroke-width="2.2" fill="none"/><path d="M78 92 q-8 -2 -6 -8 q8 0 6 8z" fill="#2f8f5b"/>
    ...
    <text x="225" y="80" text-anchor="middle" font-family="monospace" font-size="9" fill="#1c2a31">NITROGEN</text>
    <text x="225" y="92" text-anchor="middle" font-family="monospace" font-size="11" font-weight="700" fill="#2a7d8a">% N ?</text>
    <g transform="translate(296,82)"><circle cx="0" cy="0" r="18" fill="#3a98a6"/><text x="0" y="6" text-anchor="middle" font-family="monospace" font-size="16" font-weight="700" fill="#0b1a22">N</text></g>
    <g stroke="#79b0ba" stroke-width="2" fill="none" stroke-linecap="round"><path d="M268 70 q8 6 0 14"/></g>` }),

  'c-fuelpurity': scene('c-fuelpurity', { caption: 'FUEL POD · VERIFY % CARBON', body: `
    ...
    <text x="84" y="81" text-anchor="middle" font-family="monospace" font-size="13" font-weight="700" fill="#e8f2f4">CH₄</text>
    ...
    <g transform="translate(208,52)">
      <circle cx="0" cy="0" r="22" fill="#08141a" stroke="#79b0ba" stroke-width="2"/>
      <path d="M0 0 L13 -10" stroke="#2f8f5b" stroke-width="2.5" stroke-linecap="round"/>
      <path d="M-15 6 A18 18 0 0 1 15 6" fill="none" stroke="#3a5560" stroke-width="2"/>
      <circle cx="0" cy="0" r="2.5" fill="#cfe6ea"/>
    </g>
    <text x="208" y="92" text-anchor="middle" font-family="monospace" font-size="8.5" fill="#9fc2c9">PURITY</text>` }),

  // ---------- C.8(D) empirical / molecular ----------
  'd-leak': scene('d-leak', { caption: 'UNKNOWN GAS · BUILD THE FORMULA, ID IT', bg: ['#0b1a22', '#241818'], frame: '#bf4a30', capColor: '#cda99a', body: `
    ...
    <g transform="translate(214,72)"><circle r="16" fill="#241818" stroke="#bf4a30" stroke-width="2"/><text x="0" y="6" text-anchor="middle" font-family="monospace" font-size="18" font-weight="700" fill="#e08a5a">?</text></g>
    <rect x="286" y="48" width="92" height="64" rx="8" fill="#08141a" stroke="#3a5560" stroke-width="2"/>
    ...
    <text x="332" y="62" text-anchor="middle" font-family="monospace" font-size="8" fill="#e08a5a">ALERT</text>` }),

  'd-surface': scene('d-surface', { caption: 'DRILLED SOLID · EMPIRICAL THEN MOLECULAR', bg: ['#0b1a22', '#241410'], body: `
    ...
    <g transform="translate(204,70)">
      ...
      <text x="0" y="8" text-anchor="middle" font-family="monospace" font-size="16" font-weight="700" fill="#cfe6ea">?</text>
    </g>
  ...
  'd-coolant': scene('d-coolant', { caption: 'COOLANT LEAK · ID THE FLUID', body: `
    ...
    <text x="129" y="76" text-anchor="middle" font-family="monospace" font-size="11" font-weight="700" fill="#0b1a22">?</text>
    <rect x="220" y="96" width="150" height="42" rx="4" fill="#1c4a32" stroke="#2f8f5b" stroke-width="2"/>
  ...
  'h1-desiccant': scene('h1-desiccant', { theme: 'copper', caption: 'HONORS · FIND x FROM WATER LOST', body: `
    ...
    <text x="285" y="48" text-anchor="middle" font-family="monospace" font-size="13" font-weight="700" fill="#f1ddc8">·x H₂O</text>` }),

  // ---------- Honors: fire forensics (copper) ----------
  'h2-arson': scene('h2-arson', { theme: 'copper', bg: ['#1c1208', '#2e1813'], caption: 'HONORS · EMPIRICAL FROM CO₂ + H₂O', body: `
    ...
    <text x="232" y="58" text-anchor="middle" font-family="monospace" font-size="11" font-weight="700" fill="#9cc6cf">H₂O</text>
    <g><circle cx="284" cy="92" r="4.5" fill="#8a8f95"/><circle cx="276" cy="92" r="3.2" fill="#cfdbe0"/><circle cx="292" cy="92" r="3.2" fill="#cfdbe0"/></g>
    <text x="284" y="62" text-anchor="middle" font-family="monospace" font-size="11" font-weight="700" fill="#cfdbe0">CO₂</text>
  ...
  // ---------- Capstone ----------
  'cap-pod': scene('cap-pod', { caption: 'RESUPPLY POD · ID, CHECK PURITY, DECIDE', body: `
    ...
    <text x="118" y="84" font-family="monospace" font-size="20" font-weight="700" fill="#687a82">?</text>
    ...
    <g transform="translate(232,46)">
      ...
      <path d="M-16 -8 l-7 12 h14 z" fill="none" stroke="#2f8f5b" stroke-width="1.8"/>
