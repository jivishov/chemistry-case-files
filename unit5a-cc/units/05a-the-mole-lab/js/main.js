...
// Optional Mission Deck motion layer. Safe to import headless (every export no-ops
// without a DOM), so the Node createSim() verification path is unaffected.
import { surge, celebrate } from './motion.js';
...
export function createSim() {
  return {
    ...
    // to chem.lab.05a-the-mole-lab (NOT through game.js); `rungMiss` is session-only.
    rung: { a: 1, b: 1 },
    rungMiss: { a: 0, b: 0 },
    ...
    init() {
      ...
      this.genHydrate();
      this.genCombustion();
      // Mission Deck feedback layer (presentation only; no effect on grading/state).
      // Every committed call sets lastVerdict, so one watcher drives all the motion:
      // a subtle surge on the crew meter for a win, and a confetti burst the moment a
      // system newly certifies. _masteredSnap tracks which skills were already mastered.
      this._masteredSnap = this._masteredSet();
      // $watch is an Alpine magic; guard it so a headless Node import of createSim()
      // (the verification path) can still call init() without throwing.
      if (typeof this.$watch === 'function') this.$watch('lastVerdict', v => { if (v) this.onVerdict(v); });
    },

    // ---- Mission Deck motion hooks (presentation only) ----
    _masteredSet() {
      const s = new Set();
      for (const def of (this.g_skillDefs || [])) if (this.gMastered(def.id)) s.add(def.id);
      return s;
    },
    onVerdict(v) {
      const now = this._masteredSet();
      let newlyMastered = false;
      now.forEach(id => { if (!this._masteredSnap || !this._masteredSnap.has(id)) newlyMastered = true; });
      this._masteredSnap = now;
      // The consequence verdict AND the self-explanation panel both render on the
      // always-visible status column now, so nothing needs scrolling into view; just
      // surge the crew meter on a win (brightness pulse; clip-safe).
      if (typeof document !== 'undefined' && v.tone === 'success') {
        this.$nextTick(() => { const el = document.querySelector('[data-crew-meter]'); if (el) surge(el); });
      }
      // celebrate only on a NEW certification; a bigger burst when the whole mission is done
      if (v.tone === 'success' && newlyMastered) celebrate({ big: this.gOverall() === 1 });
    },

    // The active stage's verdict, surfaced on the Life-support column so a commit's
    // consequence is ALWAYS visible without scrolling the task (the inline stage verdict
    // is hidden in the cockpit). Reactive: clears automatically via each stage's *Verdict.
    get stageVerdict() {
      if (this.mode === 'molg' || this.mode === 'particles') return this.auditVerdict || this.cvVerdict;
      if (this.mode === 'percent')  return this.pcVerdict;
      if (this.mode === 'formula')  return this.hyVerdict || this.cbVerdict || this.foVerdict;
    ...
    resetProgress() {
      ...
      if (typeof localStorage !== 'undefined') {
        try { localStorage.removeItem('chem.lab.05a-the-mole-lab'); } catch { /* ignore */ }
      }
    ...
    labLoad() {
      ...
      try {
        const raw = localStorage.getItem('chem.lab.05a-the-mole-lab');
        if (!raw) return;
    ...
    labSave() {
      if (typeof localStorage === 'undefined') return;
      try {
        localStorage.setItem('chem.lab.05a-the-mole-lab',
    ...
    recordWorld({ icon, tone, text, stock, delta }) {
      ...
      if (this.worldLog.length > 4) this.worldLog.pop();
    },
    recomputeCrew() {
    ...
    get cvDecadeMismatch() {
      ...
      // At rung 2 a magnitude check is only meaningful once the units cancel; a flipped
      // factor is handled by the units feedback (STALLED + the flipped self-explanation),
      // not by a misleading magnitude nag.
      if (this.cv.rung === 2 && !this.cvBuiltFactor.cancels) return false;
    ...
    cvDetectMiss(value) {
      ...
      // 1. flipped: units don't cancel. At rung 2 that is a non-canceling built factor; at
      //    rung 1 it is a chain that never reached the target unit. (No factor at rung 3.)
      if (this.cv.rung === 2) { if (!this.cvBuiltFactor.cancels) return 'flipped'; }
      else if (this.cv.rung === 1) { if (!this.cvSteps.reached) return 'flipped'; }
