...
// scientific definitions and measurements.
...
} from './model.js?v=u1-2';
import { sceneArt } from './art.js?v=u1-3';
import {
...
const MAXV = 50;
const GLASS_X = 48, GLASS_W = 90;
const CX_L = 50, CX_R = 136, CX_MID = (CX_L + CX_R) / 2;
const TOP = 66, BOT = 566, SPAN = BOT - TOP;
const MENISCUS = 6;

// Reference datasets used for accuracy/precision practice.
const EV_SCENARIOS = [
...
const skills = [
  { id: 'a',  code: 'Practice 1', label: 'Reading the tool',        target: 3 },
  { id: 'b',  code: 'Practice 2', label: 'Significant figures',     target: 3 },
  { id: 'c',  code: 'Practice 3', label: 'Density by displacement', target: 3 },
  { id: 'd',  code: 'Practice 4', label: 'Accuracy vs precision',   target: 3 },
  { id: 'h1', code: 'Honors',     label: 'Maximum uncertainty',     target: 2, honors: true },
  { id: 'h2', code: 'Honors',     label: 'Standard deviation',      target: 2, honors: true },
...
export function createSim() {
  return {
    ...
    // Session-local simulation state.
    ...
    recordWorld({ icon, tone, text, effect }) {
      this.day++;
      const e = effect || {};
      // Baseline drift and couplings are simulation mechanics, not measured aquarium laws.
    ...
    get fishMood() { return this.fishHealth >= 67 ? '\u{1F41F}' : this.fishHealth >= 34 ? '\u{1F630}' : '\u{1F6A8}'; },
    get tankState() {
      return this.fishHealth >= 67 ? 'Water conditions stable'
        : this.fishHealth >= 34 ? 'Water conditions need attention'
          : 'Water conditions unsafe';
    },
    stockColor(v) { return v >= 67 ? 'var(--success)' : v >= 34 ? 'var(--warn)' : 'var(--danger)'; },
    get tankReadings() {
      return [
        { key: 'water', label: 'Chlorine', raw: `${this.tank.chlorine.toFixed(2)} mg/L`, pct: this.sWater,
          hint: `free chlorine — simulation stress threshold ${TANK.stressLine.toFixed(2)} mg/L` },
        { key: 'metals', label: 'Dissolved metal', raw: `${this.tank.metals.toFixed(3)} mg/L`, pct: this.sMetals,
          hint: `simulation alert threshold ${TANK.metalLine.toFixed(3)} mg/L; metal effects are simplified for this activity` },
        { key: 'log', label: 'Record reliability', raw: `${Math.round(this.tank.log)}%`, pct: this.tank.log,
          hint: 'simulation score for the reliability of the written record' },
        { key: 'kit', label: 'Test-kit reliability', raw: `${Math.round(this.tank.kit)}%`, pct: this.tank.kit,
          hint: 'simulation score for how reliable the current test kit is' }
      ];
    },
    ...
    get activeStationName() {
      ...
      if (this.mode === 'capstone') return 'Water-change challenge';
      return 'Tank status';
    },
    ...
    get activeOutcomeText() {
      ...
      return 'Choose an activity and follow the measurement or data-analysis steps.';
    },
    ...
    get activeReference() {
      ...
      if (this.mode === 'measure') {
        ...
        out.push({ k: 'Report to', v: '0.1 mL — one estimated digit beyond the marks' });
      } else if (this.mode === 'sigfig') {
        out.push({ k: 'Leading zeros', v: 'not significant' });
        out.push({ k: 'Trailing zeros', v: 'after a decimal: significant; whole-number zeros may be ambiguous' });
        out.push({ k: 'Products / quotients', v: 'use the fewest significant figures in the inputs' });
      } else if (this.mode === 'density') {
        out.push({ k: 'Density', v: 'ρ = m ÷ V' });
        out.push({ k: 'Displacement', v: 'V = after − before' });
        out.push({ k: 'Activity reference', v: 'remove zinc, copper, and lead in this simulation' });
      } else if (this.mode === 'evaluate' && this.evScenario) {
        out.push({ k: 'Reference value', v: `${this.evAccepted} ${this.evScenario.unit}`.trim() });
        out.push({ k: 'Percent error', v: '|mean − reference| ÷ reference × 100' });
        out.push({ k: 'Activity criteria', v: 'accurate: <2.5% error; precise: <2% relative SD' });
        ...
        out.push({ k: 'Test-kit score', v: `${Math.round(this.tank.kit)}% (simulation)` });
      }
    ...
    // ================= C.1: reading the graduated cylinder =================
    get ticks() {
      const out = [];
    ...
    newMeasure() {
      ...
      const base = 6 + ((Math.random() * 40) | 0);
      const tenth = 1 + ((Math.random() * 9) | 0);
    ...
    get mGauge() {
      ...
      return {
        ...
        ref: this.mVal, refLabel: 'reference level', unit: ' mL', digits: 3,
        label: 'your reading compared with the reference level'
      };
    },
    logDose() {
      ...
      let v, effect;
      if (good) {
        v = { tone: 'success', icon: sc.icon, state: sc.safeState, headline: 'Correct reading',
          detail: `You recorded ${val.toFixed(1)} mL. The reference level is ${this.mVal.toFixed(1)} mL. ${sc.safe}` };
        effect = sc.effect.good;
        this.mDone = true;
      } else if (band.direction === 'low') {
        v = { tone: 'fail', icon: '\u{1F4CF}', state: sc.lowState, headline: 'Reading is too low',
          detail: `You recorded ${val.toFixed(1)} mL. The reference level is ${this.mVal.toFixed(1)} mL, so the reading is ${off} mL low. ${sc.low}` };
        effect = sc.effect.low;
      } else {
        v = { tone: 'fail', icon: '\u{1F4CF}', state: sc.highState, headline: 'Reading is too high',
          detail: `You recorded ${val.toFixed(1)} mL. The reference level is ${this.mVal.toFixed(1)} mL, so the reading is ${off} mL high. ${sc.high}` };
        effect = sc.effect.high;
      }
      this.gRecord('a', good, !this.mAttempted);
      this.recordWorld({ icon: v.icon, tone: v.tone, text: `${sc.system}: ${good ? 'reading correct' : 'reading needs review'}`, effect });
    ...
    get sfReady() { return this.sfTask === 'count' ? this.sfPick !== null : this.sfInput !== ''; },
    get sfCorrect() {
    ...
    writeLog() {
      ...
      const v = good
        ? { tone: 'success', icon: sc.icon, state: 'CORRECT PRECISION', headline: 'Significant figures are correct',
          detail: `${truth} is the correct response. ${sc.success}` }
        : { tone: 'fail', icon: '\u{270F}\u{FE0F}', state: 'CHECK PRECISION', headline: 'Check the significant figures',
          detail: `You entered ${said}; the correct response is ${truth}. ${sc.fail}` };
      this.gRecord('b', good, !this.sfAttempted);
      this.recordWorld({ icon: v.icon, tone: v.tone, text: `${sc.system}: ${good ? 'precision recorded correctly' : 'precision needs review'}`, effect: good ? sc.effect.good : sc.effect.bad });
    ...
    get dGauge() {
      ...
      return {
        ...
        ref: this.dDensTrue, refLabel: 'reference density for the sample', unit: ' g/mL', digits: 3,
        bands, label: 'your calculated density'
      };
    },

    // Simplified maximum relative uncertainty estimate used by the Honors activity.
    get dUncert() {
      const dm = 0.1, dv = 0.2;
    ...
    certifySample() {
      ...
      let v, effect;
      if (good) {
        v = { tone: 'success', icon: sc.icon, state: sub.toxic ? 'SAMPLE REMOVED' : 'SAMPLE CLEARED', headline: 'Identification supported by the data',
          detail: `${fmt(yours, 3)} g/mL identifies ${sub.name}. ${sc.consequences[this.dAction]}` };
        effect = { ...sc.effect.good };
        this.dDone = true;
      } else {
        const leftToxic = sub.toxic && this.dAction === 'keep';
        const why = !densOk ? `Your calculated density (${this.dDensInput}) differs from the reference value ${fmt(this.dDensTrue, 3)} g/mL. Recheck the displacement volume and ρ = m/V.`
          : !idOk ? `The reference density ${fmt(this.dDensTrue, 3)} g/mL identifies ${sub.name}, not ${this.dPick}.`
            : `According to this activity's compatibility reference, ${sub.name} ${sub.toxic ? 'should be removed' : 'may remain'} in the simulated tank.`;
        v = { tone: 'fail', icon: leftToxic ? '\u{26A0}\u{FE0F}' : '\u{1F50E}', state: leftToxic ? 'REMOVE SAMPLE' : 'CHECK IDENTIFICATION',
          headline: leftToxic ? 'The activity reference indicates this sample should be removed' : 'The conclusion is not supported',
      ...
      this.recordWorld({ icon: v.icon, tone: v.tone, text: `${sc.system}: ${good ? 'identification supported' : 'identification needs review'}`, effect });
    ...
    certifyH1() {
      ...
      const v = good
        ? { tone: 'success', icon: sc.icon, state: 'UNCERTAINTY CHECKED', headline: 'The uncertainty supports the decision',
          detail: `Estimated maximum relative uncertainty: ±${fmt(this.dUncert.rel, 2)}%. Gap to the nearest candidate: ${fmt(this.dGapPct, 2)}%. ${sc.consequences[this.h1Pick]}` }
        : { tone: 'fail', icon: '\u{2696}\u{FE0F}', state: numOk ? 'CHECK DECISION' : 'CHECK CALCULATION',
          headline: numOk ? 'The uncertainty value is correct; review the decision' : 'Check the maximum relative uncertainty',
          detail: numOk
            ? `Compare ±${fmt(this.dUncert.rel, 2)}% with the ${fmt(this.dGapPct, 2)}% gap. The supported choice is "${this.h1True === 'call' ? 'identify the metal' : 'use a larger sample'}".`
            : `Using the activity's maximum-uncertainty method: 0.1 g ÷ ${fmt(this.dMass, 3)} g plus 0.2 mL ÷ ${fmt(this.dVolTrue, 3)} mL = ${fmt(this.dUncert.rel, 2)}%.` };
      this.gRecord('h1', good, !this.h1Attempted);
      this.recordWorld({ icon: v.icon, tone: v.tone, text: `Uncertainty evaluation: ${good ? 'supported by the data' : 'needs review'}`, effect: good ? sc.effect.good : sc.effect.bad });
    ...
    // These cutoffs are activity criteria, not definitions of accuracy or precision.
    ...
    get evGauge() {
      ...
      return {
        ...
        ref: a, refLabel: 'reference value', unit: this.evScenario.unit ? ` ${this.evScenario.unit}` : '', digits: 3,
        label: 'five-trial mean compared with the reference'
      };
    },
    judgeKit() {
      ...
      const v = good
        ? { tone: 'success', icon: sc.icon, state: 'CORRECT CLASSIFICATION', headline: `${truth.title}`,
          detail: `${fmt(this.evPctError, 2)}% error and ${fmt(this.evRelStd, 2)}% relative standard deviation. ${sc.consequences[this.evVerdictKey]}` }
        : { tone: 'fail', icon: '\u{1F4C9}', state: 'CHECK CLASSIFICATION', headline: `This dataset is ${truth.title.toLowerCase()}`,
          detail: `${fmt(this.evPctError, 2)}% error means it is ${this.evAccurate ? 'accurate' : 'not accurate'} by this activity's criterion; ${fmt(this.evRelStd, 2)}% relative standard deviation means it is ${this.evPrecise ? 'precise' : 'not precise'}. ${sc.consequences[this.evClassPick]}` };
      this.gRecord('d', good, !this.evAttempted);
      this.recordWorld({ icon: v.icon, tone: v.tone, text: `${sc.system}: ${good ? 'classification correct' : 'classification needs review'}`, effect: good ? sc.effect.good : sc.effect.bad });
    ...
    get h2True() {
      if (!this.evPrecise) return 'replace';
      if (!this.evAccurate) return 'send';
      return 'trust';
    },
    ...
    certifyH2() {
      ...
      const good = numOk && callOk;
      const v = good
        ? { tone: 'success', icon: sc.icon, state: 'EVALUATION COMPLETE', headline: 'Standard deviation calculated and evidence evaluated',
          detail: `s = ${fmt(this.evStd, 3)} for a mean of ${fmt(this.evMean, 4)}. ${sc.consequences[this.h2Pick]}` }
        : { tone: 'fail', icon: '\u{1F4C9}', state: numOk ? 'CHECK DECISION' : 'CHECK CALCULATION',
          headline: numOk ? 'The standard deviation is correct; review the next step' : 'Check the standard deviation',
          detail: numOk
            ? `The set is ${this.evPrecise ? 'precise' : 'not precise'} and ${this.evAccurate ? 'accurate' : 'not accurate'} by this activity's criteria. Use those two results to choose the next step.`
            : `s = √(Σ(xᵢ − x̄)² ÷ (n−1)) for the five trials is ${fmt(this.evStd, 3)}.` };
      this.gRecord('h2', good, !this.h2Attempted);
      this.recordWorld({ icon: v.icon, tone: v.tone, text: `Test-kit evaluation: ${good ? 'supported by the data' : 'needs review'}`, effect: good ? sc.effect.good : sc.effect.bad });
    ...
    get capGauge() {
      ...
      return {
        ...
        ref: TANK.stressLine, refLabel: 'simulation stress threshold', unit: ' mg/L', digits: 2,
        label: 'calculated chlorine after the change'
      };
    },
    capCommit() {
      ...
      let v, effect;
      if (good) {
        v = { tone: 'success', icon: sc.icon, state: 'DECISION SUBMITTED', headline: 'Calculation and decision are consistent',
          detail: `A ${this.cap.change}% water change reduces the calculated free chlorine from ${this.cap.before.toFixed(2)} mg/L to ${this.cap.after.toFixed(2)} mg/L. ${sc.consequences[this.capPick]}` };
        effect = sc.effect.good;
        this.capWin = true;
      } else {
        const dangerous = this.capPick === 'tonight' && this.cap.correct !== 'tonight';
        v = { tone: 'fail', icon: dangerous ? '\u{26A0}\u{FE0F}' : '\u{23F3}', state: numOk ? 'CHECK DECISION' : 'CHECK CALCULATION',
          headline: numOk ? 'The chlorine calculation is correct; review the decision' : 'Check the chlorine calculation',
          detail: numOk
            ? `${this.cap.after.toFixed(2)} mg/L chlorine, ${this.cap.metals.toFixed(3)} mg/L dissolved metal, test-kit reliability score ${Math.round(this.cap.kit)}%. ${sc.consequences[this.capPick]}`
      ...
      this.recordWorld({ icon: v.icon, tone: v.tone, text: `Water change: ${good ? 'decision supported by the data' : 'decision needs review'}`, effect });
