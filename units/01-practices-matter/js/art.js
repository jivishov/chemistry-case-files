...
const WATER_BG = ['#0a2730', '#0e3b44'];
const ROOM_BG  = ['#141b21', '#232c33'];
...
export const SCENE_ART = {
  'a-dechlor': scene('a-dechlor', { caption: 'DECHLORINATOR · READ AND RECORD THE VOLUME', body: k => {
  ...
  'a-plantfood': scene('a-plantfood', { caption: 'PLANT NUTRIENT · READ THE MENISCUS', body: k => {
  ...
  'a-meds': scene('a-meds', { caption: 'AQUARIUM TREATMENT · MEASURE THE VOLUME', body: k => {
    ...
    return `<g opacity=".5">${waterColumn('34,26,.9 356,34,1')}</g>`
      ...
      + mono(320, 91, 'TARGET AMOUNT', { size: 6.5, fill: C.dim, ls: '.06em' })
      + `<rect x="272" y="96" width="96" height="9" rx="4" fill="#1c3b44"/>`
      + `<rect x="304" y="96" width="30" height="9" rx="4" fill="${C.success}" opacity=".85"/>`;
  } }),

  'b-log': scene('b-log', { theme: 'room', caption: 'SIGNIFICANT FIGURES · REPORT PRECISION', body: k => {
  ...
  'b-volume': scene('b-volume', { theme: 'room', caption: 'LABELED VOLUME · MEASURED VOLUME', body: k => {
    ...
    return deskShelf(118)
      ...
      + mono(329, 47, 'LABEL', { size: 7, fill: C.dim, ls: '.08em' })
      + mono(329, 59, '20 gal', { size: 13, fill: C.warn, w: 700 })
      + panelBox(276, 70, 106, 30, { stroke: C.teal })
      + mono(329, 83, 'MEASURED', { size: 7, fill: C.dim, ls: '.08em' })
      + mono(329, 95, '18.4 gal', { size: 13, fill: C.white, w: 700 });
  } }),

  'b-pergallon': scene('b-pergallon', { theme: 'room', caption: 'CALCULATOR DISPLAY · USE CORRECT SIG FIGS', body: k => {
    ...
    return deskShelf(120)
      ...
      + mono(349, 58, 'REPORT AS', { size: 7, fill: C.dim, ls: '.08em' })
      + mono(349, 74, '92 mL', { size: 15, fill: C.white, w: 700 });
  } }),

  'c-ornament': scene('c-ornament', { caption: 'MASS → VOLUME → DENSITY', body: k => {
  ...
  'c-pendant': scene('c-pendant', { caption: 'IDENTIFY THE METAL FROM DENSITY', body: k => {
  ...
  'c-anchor': scene('c-anchor', { caption: 'UNLABELED SAMPLE · IDENTIFY BY DENSITY', body: k => {
  ...
  'd-dropkit': scene('d-dropkit', { theme: 'room', caption: 'FIVE TRIALS · ONE REFERENCE VALUE', body: k => {
    return deskShelf(120)
      ...
      + mono(212, 102, 'ACCURATE + PRECISE', { size: 7, fill: C.dim, ls: '.04em' })
      + targetBoard(318, 56, 34, [[.5, .38], [.58, .46], [.46, .5], [.54, .42], [.6, .36]], C.danger)
      + mono(318, 102, 'PRECISE, NOT ACCURATE', { size: 6.5, fill: C.dim, ls: '.02em' })
      + `<path d="M256 56 h24" stroke="${C.steelLt}" stroke-width="1.2" stroke-dasharray="3 4" opacity=".6"/>`
      + mono(268, 40, 'or', { size: 9, fill: C.dim });
  } }),

  'd-penmeter': scene('d-penmeter', { theme: 'room', caption: 'DISPLAY RESOLUTION ≠ PRECISION', body: k => {
  ...
  'd-strips': scene('d-strips', { theme: 'room', caption: 'HEAT-EXPOSED TEST STRIPS', body: k => {
  ...
  'h1-sizecall': scene('h1-sizecall', { theme: 'copper', caption: 'IS THE UNCERTAINTY SMALL ENOUGH?', body: k => {
    ...
    return `<g opacity=".35">${deskShelf(124)}</g>`
      ...
      + mono(247, 22, 'ESTIMATED RANGE', { size: 7.5, fill: C.copper1, ls: '.08em', w: 700 });
  } }),

  'h2-kitcall': scene('h2-kitcall', { theme: 'copper', caption: 'SCATTER AND BIAS · DIFFERENT RESPONSES', body: k => {
    return `<g opacity=".35">${deskShelf(124)}</g>`
      + targetBoard(86, 46, 30, [[.5, -.12], [-.44, .22], [.1, .54], [-.2, -.5], [.16, -.2]], C.ember)
      + mono(86, 88, 'SCATTER', { size: 8, fill: C.copper1, ls: '.1em', w: 700 })
      + mono(86, 99, 'check reagent / technique', { size: 7, fill: '#c39a6a' })
      + targetBoard(314, 46, 30, [[.52, .4], [.6, .48], [.48, .52], [.56, .44], [.62, .38]], C.danger)
      + mono(314, 88, 'BIAS', { size: 8, fill: C.copper1, ls: '.1em', w: 700 })
      + mono(314, 99, 'independent reference', { size: 7, fill: '#c39a6a' })
      ...
      + mono(200, 88, 'STANDARD DEVIATION', { size: 6.5, fill: '#c39a6a', ls: '.04em' });
  } }),

  'cap-waterchange': scene('cap-waterchange', { caption: 'USE THE DATA · CHOOSE THE NEXT STEP', body: k => {
