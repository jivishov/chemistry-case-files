...
export function createSim() {
  return {
    ...
    setMode(m) {
      ...
      // The three charts and the WebGL box are created at module scope. Re-size after
      // their newly-visible cockpit panel paints rather than storing renderer state in Alpine.
      this.$nextTick(() => { this.resizeCharts(); gasbox?.resize?.(); });
    },
    ...
    get activeBrief() {
      const byMode = {
        kmt: this.kq && this.kq.sc, ideal: this.ig && this.ig.sc,
        dalton: this.dl && this.dl.sc, capstone: (this.cap && this.cap.sc) || SCENARIOS.find(s => s.id === 'cap-lastfill')
      };
      return byMode[this.mode] || null;
    },
    get activeVerdict() {
      return ({ kmt: this.kqVerdict, ideal: this.igVerdict, dalton: this.dlVerdict, capstone: this.capVerdict })[this.mode] || null;
    },
    get activeTone() {
