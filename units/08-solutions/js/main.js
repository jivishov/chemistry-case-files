...
export function createSim() {
  return {
    ...
    setMode(m) {
      ...
      // Chart.js reads a zero-size canvas while its panel is display:none. The chart stays
      // module-scoped; this next-tick resize is the cockpit-specific part of the lifecycle.
    ...
    // ================= cockpit readouts =================
    // These getters only project existing bench state into the shell. They do not change
    // any chemistry, tolerance, scenario selection, or world-state consequence.
    ...
    get activeStationName() { return (this.activeBrief && this.activeBrief.system) || 'Clearwell watch'; },
    ...
    get activeOutcomeText() {
      ...
      return (v && (v.detail || v.headline)) || (b && (b.why || b.goal)) || 'Read the bench, make a call, and record what reaches the finished water.';
    },
    get activeReference() {
      if (this.mode === 'curve') return [{ k: 'Read', v: 'g solute per 100 g water' }, { k: 'Rate', v: 'temperature, agitation, surface area' }];
      if (this.mode === 'molarity') return [{ k: 'M', v: 'mol solute / L solution' }, { k: 'Mass', v: 'moles x molar mass' }];
      if (this.mode === 'dilute') return [{ k: 'Equation', v: 'C1V1 = C2V2' }, { k: 'Volume', v: 'use matching units' }];
      if (this.mode === 'precip') return [{ k: 'Rule', v: 'products may remain aq or form s' }, { k: 'Evidence', v: 'write a phase for each product' }];
      return [{ k: 'Plant target', v: 'manganese at or below 0.05 mg/L' }, { k: 'Water', v: 'polar solvent; like dissolves like' }];
    },
    get waterReadings() {
      const clean = this.mnPct;
      const jobs = Math.min(100, this.jobs / JOBS_PER_DAY * 100);
      return [
        { key: 'mn', label: 'Manganese', raw: `${this.mnLabel} mg/L`, pct: clean, color: this.mnColor, hint: 'finished-water manganese against the secondary standard' },
        { key: 'limit', label: 'Limit', raw: `${this.mnLimit} mg/L`, pct: 100, color: 'var(--success)', hint: 'secondary standard used for the notice' },
        { key: 'jobs', label: 'Jobs', raw: `${this.jobs}`, pct: jobs, color: 'var(--accent)', hint: 'bench calls logged this notice day' },
        { key: 'mastery', label: 'Core', raw: `${this.teksMasteredCount}/6`, pct: this.gOverall() * 100, color: 'var(--accent-700)', hint: 'core skills certified' }
      ];
    },
