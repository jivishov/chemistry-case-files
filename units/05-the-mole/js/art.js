...
// Carried here in full (not re-exported) so the units_new tree stands on its own.
...
export const SCENE_ART = {
  ...
  'd-leak': scene('d-leak', { caption: 'UNKNOWN GAS · BUILD AND COMPARE FORMULAS', bg: ['#0b1a22', '#241818'], frame: C.danger, capColor: '#cda99a', body: k => {
    ...
    return bulkhead(22)
      ...
      // Above the axis, not under it: at y=110 this label sat below the y=102 scrim line,
      // which is the one rule every body in this file keeps. The gap between the last peak
      // and the panel's right edge is empty, so it reads as an axis label either way.
      + mono(380, 98, 'm/z', { size: 6.5, anchor: 'end' });
  } }),
