...
//   D a "Feel a mole" zoom tab.
// A, B and C are view-model/UI logic that lives HERE, not in the shared engine, so
// chem.js/game.js and their Node test suites are untouched. D is the exception and it is
// deliberate: the powers-of-ten tool is identical in Unit 5 and in this build, so it comes
...
export function createSim() {
  return {
    ...
    // only expands on demand. Session-only: nothing about it is worth persisting.
    teksOpen: false,
    ...
    // to chem.lab.units_new/05-the-mole (NOT through game.js); `rungMiss` is session-only.
    rung: { a: 1, b: 1 },
    rungMiss: { a: 0, b: 0 },
    ...
    // Molecular-eyes zoom slider (ungraded). zoomPow and its readouts come from
    // createMoleZoom() above; state persists across tab switches and is not regenerated.
    ...
    resetProgress() {
      ...
      if (typeof localStorage !== 'undefined') {
        try { localStorage.removeItem('chem.lab.units_new/05-the-mole'); } catch { /* ignore */ }
      }
    ...
    labLoad() {
      ...
      try {
        const raw = localStorage.getItem('chem.lab.units_new/05-the-mole');
        if (!raw) return;
    ...
    labSave() {
      if (typeof localStorage === 'undefined') return;
      try {
        localStorage.setItem('chem.lab.units_new/05-the-mole',
    ...
    // current station. Rendered as a standing card in the mission column, so the numbers
    // stay on screen instead of living in one 12px line inside the console.
    get activeReference() {
      const out = [];
      const mode = this.mode;
      if ((mode === 'molg' || mode === 'particles') && this.cv) {
        out.push({ k: 'Molar mass', ce: this.cv.sub.f, v: `${fmt(this.cv.M)} g/mol` });
      } else if (mode === 'percent' && this.pc) {
        out.push({ k: 'Molar mass', ce: this.pc.sub.f, v: `${fmt(molarMass(this.pc.sub.f))} g/mol` });
      } else if (mode === 'formula' && this.fo) {
        out.push({ k: 'Empirical molar mass', v: `${fmt(this.fo.empMass)} g/mol` });
        out.push({ k: 'Sample molar mass', v: `${fmt(this.fo.molMass)} g/mol` });
      } else if (mode === 'capstone' && this.cap) {
        out.push({ k: 'Empirical molar mass', v: `${fmt(this.cap.empMass)} g/mol` });
        out.push({ k: 'Sample molar mass', v: `${fmt(this.cap.molMass)} g/mol` });
      }
      if (mode === 'particles' || mode === 'zoom') {
        out.push({ k: 'Avogadro’s number', v: `${AVO_TEXT} per mole` });
      }
      // Deliberately short. This card shares a column with the scenario narrative, and
      // every extra line here is a line taken off the text a learner actually reads.
    ...
    // Behaviour comes from createMoleZoom() above (shared/js/molezoom.js), not from a local
    // implementation. What that buys over the 300-circle square grid this used to build
    // inline: a HEX lattice (15.5% denser), a cap read off the real stage size and device
    // pixel ratio rather than a magic 300, and one <pattern> per row parity instead of one
    // <circle> per dot -- which is why it can stand for ~80,000 dots and still hold 60fps.
    // The markup calls these as METHODS (zoomCount(), zoomDots()); see molezoom.js on why.
