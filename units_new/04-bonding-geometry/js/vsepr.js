...
//
// The units_new build. Copied from units/04-bonding-geometry/js/vsepr.js with one addition:
// resize() is exported, because in the cockpit this canvas lives in a panel that is
// display:none until its station is opened, and an element laid out while hidden measures
// zero. The ResizeObserver does fire on the transition out of display:none, but not before
// the first paint of the newly-shown panel, so main.js calls resize() on setMode() as well.
// The path to shared/js/stage3d.js is unchanged: units_new/<slug>/js/ sits at the same
// depth below the repo root as units/<slug>/js/ did.
...
export function createViewer() {
  ...
  return { mount, setMolecule, setAutoRotate, isMounted, dispose, resize };
}
