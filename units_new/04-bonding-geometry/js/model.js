...
//
// The units_new build. Copied from units/04-bonding-geometry/js/model.js unchanged: this
// unit was retrofitted with its Scenario layer before this tree existed, so the pools, the
// nineteen SCENARIOS and the standards map are already correct and the port is a
// presentation port. What changed in this tree lives in main.js (unitId, the cockpit
// readouts), index.html (the cockpit shell) and js/art.js (new).
//
...
export const SUBSTANCE_TYPES = [
  {
    ...
    // `heat` is `mp` restated as what a bench heat test SHOWS, which is what the capstone
    // reads. It has to hold for a compound that is already a liquid at room temperature, so
    // it talks about surviving the flame rather than about a melting point.
    heat: 'still solid long after the flame is on it',
  ...
  },
  {
    ...
    heat: 'melts or boils away almost as soon as it is warmed',
  ...
  },
  {
    ...
    heat: 'a bench flame does nothing to it at all',
  ...
  },
  {
    ...
    heat: 'glows before it softens, and holds its shape until it does',
...
export const SCENARIOS = [
  ...
  { id: 'cap-underthesink', stage: 'capstone', skill: 'cap', type: 'decision',
    ...
    goal: 'One container is left under the sink. The label is gone except two element symbols. You have a heat test, a conductivity reading, and one observation nobody would forget. Work out the bond, the name, the class and the shape, then decide what happens to it.',
