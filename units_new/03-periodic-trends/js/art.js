// art.js: Unit 3 scene art: twelve 400x150 banners for the mission screen.
//
// The world is RETROFIT-U1-U4.md section 3's "The Repair Bench": phone and laptop repairs
// in your bedroom, one bench, one drawer of parts, and a supplier datasheet with a hole in
// it. Two set signatures carry the whole unit, as the brief asks: `benchTop` for the work
// happening in front of you, and `drawerTray` for what is waiting in the drawer. Honors and
// the capstone move to the copper ground the rest of the tree uses.
//
// Rules that are not style preferences:
//   * Every gradient, pattern and clip is namespaced per scene by kit(id). Alpine keeps
//     every panel in the DOM at once, so an unprefixed id bleeds from one banner into
//     another and the second scene inherits the first one's fill.
//   * Nothing below y = 100. scene() paints a caption scrim from y=102 down, so a label at
//     y=110 comes out faded and at y=116 the layout audit reports it occluded.
//   * Light from the upper left, everywhere, so a cell in one banner is shaded like the
//     cell in the next.
//   * aria-hidden on every svg: the banner is decoration and the goal text under it is the
//     authoritative description.
//
// Twelve, not the "~15" the brief estimates: that arithmetic is Unit 1's, which has four
// core skills. Unit 3 has three, so the set is 3 x 3 + h1 + h2 + cap = 12.
...
const BENCH_BG = ['#12222a', '#1d3138'];   // at the bench, under the lamp
const DRAWER_BG = ['#141a20', '#242f37'];  // in the drawer, off to the side
const COPPER_BG = ['#1c1208', '#2e2113'];  // Honors and the last call

// The saturated family palette from the unit's own stylesheet, so a tile in a banner is
// the colour that family has on the board in the right rail.
const FAM = {
...
// ---------------------------------------------------------------- paint kit
// kit(id) hands a scene its own <defs> namespace. Bodies call k.lin(...) and get back
...
function kit(id) {
  ...
  const k = {
    ...
    // A metal face lit from the upper left: highlight in, body, shadow out.
    metal(n, [hi, base, sh]) { return k.lin(n, [[0, hi], [.3, base], [1, sh]]); },
    // A standing part seen side-on, so the light wraps across x instead of down y.
    side(n, [sh, base, hi]) { return k.lin(n, [[0, sh], [.24, hi], [.55, base], [1, sh]], true); },
    orb(n, [hi, base, sh]) { return k.rad(n, [[0, hi], [.55, base], [1, sh]]); },
    // The lamp pool on the bench, and the soft bloom around anything corroding.
...
// ---------------------------------------------------------------- primitives
/* A legibility floor for in-scene labels.
 *
 * The call sites ask for 6.2 to 8 units, and the banner is a 400-unit viewBox rendered at
 * roughly the width of the mission column -- so those labels were reaching the screen at 6
 * to 8 CSS PX. The words on the drawing ("KOH electrolyte", "lost to give K+") are the part
 * of the scene that carries the chemistry, and at 7px they were decoration.
 *
 * A floor rather than a multiplier, for two reasons: it cannot make the CAPTION (10.5) or
 * the two display numerals (10, 12) grow, which is what would overflow a 48-character
 * caption past the 400-unit frame; and the 6.2-vs-8 hierarchy it flattens was invisible at
 * that size anyway. One number, and the whole set moves together.
 *
 * The rendered size still scales with the column, so the left drag handle is the other half
 * of this control: at the default 31vw the floor lands near 11px, and wider is bigger. */
const LABEL_FLOOR = 8;
...
// Deterministic solder specks and dust, so a scene redraws identically every frame.
...
// SET SIGNATURE 1: the bench you are working at, a lit surface with a front edge.
...
// SET SIGNATURE 2: the drawer, a shallow tray with compartment walls.
...
// A periodic-table tile. The unit's whole argument is that a position is a prediction, so
// the tile is the recurring object: it appears as evidence, as a question mark, and as a
// confirmed cell on the board.
...
// A supplier datasheet: ruled rows on paper, optionally with one row blacked out.
...
// A loupe over the thing you are actually reading.
const loupe = (k, cx, cy, r = 13) =>
  `<circle cx="${cx}" cy="${cy}" r="${r}" fill="${k.rad('lens', [[0, '#dff0f3', .5], [1, '#9fc6cd', .18]])}"`
  + ` stroke="${C.steelLt}" stroke-width="1.6"/>`
  + `<line x1="${cx + r * .72}" y1="${cy + r * .72}" x2="${cx + r * .72 + 9}" y2="${cy + r * .72 + 9}"`
  + ` stroke="${C.steel}" stroke-width="2.6" stroke-linecap="round"/>`;

// A cylindrical cell, standing. `frac` is how full the bright band runs.
const battCell = (k, n, x, y, w, h, { tint = '#3f6f7a', label = '' } = {}) =>
  `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="2" fill="${k.side(n, ['#1b2b31', tint, '#cfe3e6'])}"`
  + ` stroke="rgb(0 0 0 / 30%)" stroke-width=".8"/>`
...
// A fragment of circuit board with a contact pad on it.
const padBoard = (k, n, x, y, { tarnished = false, w = 46, h = 30 } = {}) =>
  `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="2" fill="${k.lin(n, [[0, '#1e4a3a'], [1, '#12301f']])}"`
  + ` stroke="rgb(0 0 0 / 35%)" stroke-width=".8"/>`
  + `<rect x="${x + 6}" y="${y + 8}" width="${w - 12}" height="9" rx="1.2"`
  + ` fill="${tarnished ? C.tarnish : C.gold}" stroke="${tarnished ? '#4a4b50' : '#a9832f'}" stroke-width=".7"/>`
  + (tarnished
...
// An edge connector: a shell with a row of pins.
const connector = (k, n, x, y, { pins = 9, plated = C.gold, corroded = false } = {}) => {
  let out = `<rect x="${x}" y="${y}" width="${pins * 6 + 8}" height="20" rx="2"`
    + ` fill="${k.lin(n, [[0, '#3a4a52'], [1, '#202c32']])}" stroke="rgb(0 0 0 / 30%)" stroke-width=".8"/>`;
...
// A waste bin, lid open or shut. `on` marks the one being chosen.
const bin = (k, n, x, y, { label = '', tint = C.slate, on = false } = {}) =>
  `<path d="M${x} ${y} L${x + 26} ${y} L${x + 23} ${y + 26} L${x + 3} ${y + 26} Z"`
  + ` fill="${k.lin(n, [[0, tint], [1, '#1b262c']])}" stroke="${on ? C.teal3 : C.slate}" stroke-width="${on ? 1.6 : .9}"/>`
...
// A small plotted series. `pts` are [0..1] pairs; `marks` are indices to circle.
const plot = (k, n, x, y, w, h, { pts, marks = [], tint = C.ember, fill = true, grid = true } = {}) => {
  ...
  pts.forEach((_, i) => {
    ...
    out += `<circle cx="${px(i).toFixed(1)}" cy="${py(i).toFixed(1)}" r="${big ? 4 : 2.2}"`
      + ` fill="${big ? C.emberLt : tint}"${big ? ` stroke="#fff" stroke-width="1"` : ''}/>`;
  });
  return out;
};

// A labelled trend arrow, the one piece of chrome every trends scene needs.
...
export const SCENE_ART = {
  // ---------------- C.5(A) how the table developed ----------------
  'a-datasheet': scene('a-datasheet', {
    caption: 'SUPPLIER DATASHEET · ONE ROW BLACKED OUT',
    body: k => benchTop(k, 90) + crumbs(4, 12)
      ...
      // The column either side of the hole, and the hole itself as a tile.
      ...
      + mono(348, 58, 'DENSITY', { size: 6.6, fill: C.steel, ls: '.1em' })
      + [34, 46, 58].map(y => `<path d="M366 ${y - 3} l3.4 3.4 l6 -7" fill="none" stroke="${C.leaf}" stroke-width="1.6"/>`).join('')
  }),

  'a-warehouse': scene('a-warehouse', {
    caption: 'TWO BINS · THE SHELF SORTED THEM BY MASS',
    theme: 'drawer',
    body: k => drawerTray(k, 16, 30, 176, 60, [88])
      // What the warehouse did: heavier part first, so the pair is swapped.
      ...
      // The second inversion, as the shelf label the next person reads.
      ...
      + mono(256, 50, 'heavier, and still first', { size: 6.8, fill: C.dim, anchor: 'start' })
      + `<rect x="250" y="66" width="132" height="24" rx="2" fill="rgb(192 119 47 / 12%)" stroke="${C.ember}" stroke-width=".8"/>`
      + mono(316, 81, 'nuclear charge, not weight', { size: 7, fill: C.emberLt })
  }),

  'a-manual': scene('a-manual', {
    caption: 'THE OLD MANUAL · THREES BY WEIGHT, THEN IT STOPS',
    body: k => benchTop(k, 92) + crumbs(9, 10)
      // An open book: two leaves with a spine shadow between them.
      ...
      // Three triads that work: the middle mass is the mean of the outer two.
      + [['Li', '6.9'], ['Na', '23.0'], ['K', '39.1']].map((e, i) =>
        tile(34, 26 + i * 22, e[0], { fill: FAM.alkali, w: 22, h: 19, sub: e[1] })).join('')
      + [['Cl', '35.5'], ['Br', '79.9'], ['I', '126.9']].map((e, i) =>
        tile(66, 26 + i * 22, e[0], { fill: FAM.halogen, w: 22, h: 19, sub: e[1] })).join('')
      + [['Ca', '40.1'], ['Sr', '87.6'], ['Ba', '137.3']].map((e, i) =>
        tile(98, 26 + i * 22, e[0], { fill: FAM['alkaline-earth'], w: 22, h: 19, sub: e[1] })).join('')
      ...
      // The fourth row, where weight stops predicting anything.
      ...
      + mono(280, 78, 'past calcium the pattern breaks', { size: 7.4, fill: '#7a3f22' })
      + mono(280, 90, 'because weight is the wrong variable', { size: 6.8, fill: C.slate })
  }),

  // ---------------- C.5(B) chemical families ----------------
  'b-remote': scene('b-remote', {
    caption: 'THE LEAKING REMOTE · WHITE CRUST ON THE CONTACTS',
    body: k => benchTop(k, 90) + crumbs(2, 10)
      // The remote, face down, battery door off.
      ...
      // The crust: a bloom plus deposits over the spring contacts.
      + `<ellipse cx="59" cy="58" rx="26" ry="17" fill="${k.glowAt('bloom', C.crust, .42)}"/>`
      + [[46, 52, 4.4], [58, 62, 5.6], [70, 55, 3.8], [52, 66, 3.2], [66, 46, 2.8]]
        .map(([cx, cy, r]) => `<circle cx="${cx}" cy="${cy}" r="${r}" fill="${C.crust}" opacity=".82"/>`).join('')
      + mono(59, 100, 'stored a whole season', { size: 6.8, fill: C.dim })
      // The cell that did it, and the family the electrolyte belongs to.
      + battCell(k, 'cell', 128, 26, 22, 56, { tint: '#5a6b3f', label: 'alkaline AA' })
      + mono(186, 32, 'KOH electrolyte', { size: 8, fill: C.steelLt, anchor: 'start' })
      + mono(186, 44, 'Group 1 · 1 valence e-', { size: 7.4, fill: C.dim, anchor: 'start' })
      + mono(186, 56, 'lost to give K+', { size: 7.4, fill: C.dim, anchor: 'start' })
      + tile(186, 64, 'K', { fill: FAM.alkali, sub: '+1' })
      + arrow(220, 262, 76, { label: 'crust' })
      + `<path d="M300 30 a26 26 0 1 0 26 26" fill="none" stroke="${C.leaf}" stroke-width="1.6"/>`
      + mono(330, 42, 'neutralise', { size: 7.4, fill: C.leaf })
      + mono(330, 54, 'and warn them', { size: 7.4, fill: C.leaf })
      + mono(330, 74, 'the family predicted', { size: 6.8, fill: C.dim })
      + mono(330, 85, 'the failure', { size: 6.8, fill: C.dim })
  }),

  'b-contact': scene('b-contact', {
    caption: 'TWO CONTACTS, SAME FAMILY · ONE WENT BLACK',
    body: k => benchTop(k, 90) + crumbs(6, 12)
      ...
      + mono(59, 91, 'reactivity 8', { size: 6.8, fill: C.ember })
      + padBoard(k, 'right', 108, 30, { tarnished: false, w: 58, h: 38 })
      + mono(137, 80, 'gold plated', { size: 7.4, fill: C.dim })
      + mono(137, 91, 'reactivity 3', { size: 6.8, fill: C.tealLt })
      + loupe(k, 150, 42, 12)
      // Same column of the table, which is why they conduct alike.
      ...
      + arrow(272, 306, 42, { label: 'same family' })
      + arrow(272, 306, 74, { label: 'different row', color: C.emberLt })
      + mono(352, 38, 'conducts', { size: 7.4, fill: C.tealLt })
      + mono(352, 49, 'the same', { size: 7.4, fill: C.tealLt })
      + mono(352, 70, 'tarnishes', { size: 7.4, fill: C.emberLt })
      + mono(352, 81, 'differently', { size: 7.4, fill: C.emberLt })
  }),

  'b-plastic': scene('b-plastic', {
    caption: 'FLAME-RETARDANT STAMP · WHERE THE OFFCUTS GO',
    theme: 'drawer',
    body: k => drawerTray(k, 14, 26, 150, 66)
      // The bottom case, with the moulded stamp inside a recycling triangle.
      ...
      // The offcuts on their way somewhere.
      + [[176, 46, 9], [192, 62, 7], [172, 70, 6]].map(([x, y, s]) =>
        `<path d="M${x} ${y} l${s} -${s * .5} l${s * .6} ${s} l-${s * .8} ${s * .5} Z" fill="#3d4a52" stroke="${C.slate}" stroke-width=".7"/>`).join('')
      + tile(212, 24, 'Br', { fill: FAM.halogen, sub: '-1' })
      + mono(225, 62, 'Group 17', { size: 7.2, fill: C.dim })
      + mono(225, 73, '7 valence e-', { size: 7.2, fill: C.dim })
      + mono(225, 84, 'gains one', { size: 7.2, fill: C.dim })
      + bin(k, 'ewaste', 288, 42, { label: 'electrical', tint: '#2c4a55', on: true })
      ...
      + mono(330, 20, 'reactive enough to matter', { size: 6.6, fill: C.tealLt })
  }),

  // ---------------- C.5(C) periodic trends ----------------
  'c-cell': scene('c-cell', {
    caption: 'LITHIUM, NOT SODIUM · SAME GROUP, DIFFERENT SIZE',
    body: k => benchTop(k, 92) + crumbs(11, 10)
      // A phone with its cell out on the bench beside it.
      ...
      + mono(43, 100, 'the phone', { size: 6.8, fill: C.dim })
      // The two candidates, drawn to their real relative radii.
      ...
      + mono(118, 82, '6.9 u', { size: 6.8, fill: C.tealLt })
      ...
      + mono(186, 82, '23.0 u', { size: 6.8, fill: C.emberLt })
      + mono(152, 22, 'GROUP 1 · BOTH +1', { size: 6.6, fill: C.steel, ls: '.1em' })
      + arrow(212, 246, 46, { label: 'per kilogram' })
      // The cells themselves, sized by what that mass difference costs.
      + battCell(k, 'small', 262, 30, 24, 44, { tint: '#3f6f7a', label: 'Li cell' })
      + battCell(k, 'big', 306, 18, 24, 68, { tint: '#6b5b3f', label: 'Na cell, same charge' })
      + mono(370, 46, '3x', { size: 12, fill: C.emberLt, w: 600 })
      + mono(370, 58, 'mass', { size: 6.8, fill: C.dim })
  }),

  'c-connector': scene('c-connector', {
    caption: 'GOLD ON THE CONNECTOR · CORROSION DECIDES IT',
    body: k => benchTop(k, 92) + crumbs(7, 12)
      ...
      + mono(55, 77, 'works today', { size: 6.8, fill: C.emberLt })
      + mono(55, 88, 'intermittent in a year', { size: 6.4, fill: C.ember })
      + connector(k, 'au', 152, 34, { pins: 9, plated: C.gold })
      + mono(183, 66, 'gold plated', { size: 7.4, fill: C.dim })
      + mono(183, 77, 'still bright', { size: 6.8, fill: C.tealLt })
      + loupe(k, 60, 40, 11)
      // The property that actually decides, as two bars rather than a squiggle.
      + `<rect x="256" y="18" width="126" height="72" rx="3" fill="rgb(255 255 255 / 5%)" stroke="${C.slate}" stroke-width=".8"/>`
      + mono(319, 28, 'REACTIVITY INDEX', { size: 6.4, fill: C.steel, ls: '.1em' })
      ...
      + mono(378, 44, 'EN 1.9', { size: 6.6, fill: C.dim, anchor: 'end' })
      + mono(378, 72, 'EN 2.5', { size: 6.6, fill: C.dim, anchor: 'end' })
  }),

  'c-case': scene('c-case', {
    caption: 'ALUMINIUM, NOT STEEL · MASS AND WHAT RUST DOES',
    theme: 'drawer',
    body: k => drawerTray(k, 12, 20, 186, 72, [96])
      // Two panels leaning in the drawer, one already going.
      ...
      + mono(59, 100, 'Al · 27.0 u', { size: 7, fill: C.tealLt })
      + `<path d="M116 34 L182 28 L178 86 L120 88 Z" fill="${k.lin('fe', [[0, '#9aa3a6'], [1, '#5c6467']])}" stroke="rgb(0 0 0 / 30%)" stroke-width=".9"/>`
      // Rust blooming out of the screw holes, which is the whole argument.
      + [[134, 46], [162, 58], [140, 74]].map(([cx, cy]) =>
        `<circle cx="${cx}" cy="${cy}" r="7" fill="${k.glowAt('r' + cx, C.rust, .55)}"/>`
        + `<circle cx="${cx}" cy="${cy}" r="2.2" fill="#2b3134"/>`).join('')
      + mono(149, 100, 'Fe · 55.8 u', { size: 7, fill: C.ember })
      + arrow(208, 242, 40, { label: 'mass' })
      + arrow(208, 242, 72, { label: 'corrosion', color: C.tealLt })
      // The oxide layer, drawn as the thing that protects rather than the thing that fails.
      + `<rect x="254" y="26" width="128" height="26" rx="2" fill="rgb(79 163 174 / 14%)" stroke="${C.teal3}" stroke-width=".8"/>`
      + mono(258, 36, 'Al2O3 seals the surface', { size: 7.2, fill: C.tealLt, anchor: 'start' })
      + mono(258, 47, 'and stays put', { size: 6.8, fill: C.dim, anchor: 'start' })
      + `<rect x="254" y="60" width="128" height="26" rx="2" fill="rgb(141 75 42 / 16%)" stroke="${C.rust}" stroke-width=".8"/>`
      + mono(258, 70, 'Fe2O3 flakes off', { size: 7.2, fill: C.emberLt, anchor: 'start' })
      + mono(258, 81, 'and takes the screw hole', { size: 6.8, fill: C.dim, anchor: 'start' })
  }),

  // ---------------- Honors ----------------
  'h1-shielding': scene('h1-shielding', {
    caption: 'PERIOD 3 · THE ROW TIGHTENS AS Zeff CLIMBS',
    theme: 'copper',
    body: k => {
      // Eight shells drawn to their real relative radii, so the shrink is the data.
      const row = [['Na', 190], ['Mg', 145], ['Al', 118], ['Si', 111], ['P', 98], ['S', 88], ['Cl', 79], ['Ar', 71]];
      const zeff = [2.2, 2.85, 3.5, 4.15, 4.8, 5.45, 6.1, 6.75];
      let out = mono(200, 11, 'Na  →  Ar     SAME SHELL, MORE PROTONS', { size: 7, fill: '#e0b483', ls: '.08em' });
      row.forEach(([sym, r], i) => {
        ...
        out += `<circle cx="${cx}" cy="50" r="${R.toFixed(1)}" fill="${k.orb('s' + i, ['#f0cfa8', '#b0762f', '#4a2c0c'])}"/>`
          ...
          + mono(cx, 84, zeff[i].toFixed(2), { size: 6.6, fill: C.emberLt });
      });
      // One legend line rather than two axis labels: at this scale a label beside the first
      // shell sits inside it, which is what the first draft of this banner did.
      out += arrow(34, 364, 24, { label: 'radius falls, IE rises', color: '#e0b483' })
        + mono(200, 97, 'top row pm  ·  bottom row Zeff', { size: 6.4, fill: '#a98a67', ls: '.06em' });
      return out;
    }
  }),

  'h2-dip': scene('h2-dip', {
    caption: 'PERIOD 2 IONIZATION · IT CLIMBS, AND DROPS TWICE',
    theme: 'copper',
    body: k => {
      // Li Be B C N O F Ne, normalised from the unit's own ie1 values (520..2081).
      ...
      // Name each dip where it happens rather than in a legend.
      out += mono(132, 34, 'Be → B', { size: 7, fill: '#f3c98a' })
        + mono(132, 44, '2p leaves', { size: 6.4, fill: '#c9a377' })
        + mono(258, 30, 'N → O', { size: 7, fill: '#f3c98a' })
        + mono(258, 40, 'pairing costs', { size: 6.4, fill: '#c9a377' })
  ...
  'cap-substitute': scene('cap-substitute', {
    ...
    body: k => benchTop(k, 96)
      ...
      + mono(174, 18, 'mean of the two', { size: 6.4, fill: '#e0b483' })
      + mono(174, 74, 'Mendeleev’s move', { size: 6.8, fill: '#c9a377' })
      // The spring contact in its bag, which is the part the call is actually about.
